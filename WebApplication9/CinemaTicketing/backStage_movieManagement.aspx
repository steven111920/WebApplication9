<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="backStage_movieManagement.aspx.cs" Inherits="CinemaTicketingSystem.backStage_movieManagement" %>
	...
	<div id="content">
		...
		<div id="content2">
			<div class="movie_manage">
				...
				<tr height="35px">
					<td><%=moviesList[(int)Session["j"]].getMovieNo()%></td>
					<td><%=moviesList[(int)Session["j"]].getMovieName()%></td>
					<td><%=moviesList[(int)Session["j"]].getType()%></td>
					<td><%=moviesList[(int)Session["j"]].getCountry()%></td>
					<td><%=moviesList[(int)Session["j"]].getMinute()%></td>
					<td><%=moviesList[(int)Session["j"]].getDirector()%></td>
					<td><%=moviesList[(int)Session["j"]].getActors()%></td>
					<td><%=moviesList[(int)Session["j"]].getPlayDate()%></td>
					<td><%=moviesList[(int)Session["j"]].getPrice()%></td>
					<td><a href="backStage_movieUpdate.aspx?movieNo=<%=moviesList[(int)Session["j"]].getMovieNo()%>">编辑</a></td>
					<td><a href="backStage_movieManagement.aspx?deleteMovieNo=<%=moviesList[(int)Session["j"]].getMovieNo()%>">删除</a></td>
				</tr>
                <%} %>
				</table>
			</div>
