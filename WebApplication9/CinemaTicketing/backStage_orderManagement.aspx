<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="backStage_orderManagement.aspx.cs" Inherits="CinemaTicketingSystem.backStage_orderManagement" %>
		...
		<div id="content2">
			<div class="movie_manage">
                ...
                <%
                 ...
                 %>
				<tr height="35px">
					<td><%=ordersList[(int)Session["j"]].getOrderNo() %></td>
					<td><%=ordersList[(int)Session["j"]].getMovieName()%></td>
					<td><%=ordersList[(int)Session["j"]].getCustomerName()%></td>
					<td><%=ordersList[(int)Session["j"]].getPlayingDate()%></td>
					<td><%=ordersList[(int)Session["j"]].getTime()%></td>
					<td><%=ordersList[(int)Session["j"]].getHallName()%></td>
					<td><%=ordersList[(int)Session["j"]].getSeat()%></td>
					<td><%=ordersList[(int)Session["j"]].getPrice()%>元</td>
					<td><%=ordersList[(int)Session["j"]].getStatement()%></td>
					<td><%=ordersList[(int)Session["j"]].getGenerateDate()%></td>
					<td><a href="backStage_orderManagement.aspx?deleteOrderNo=<%=ordersList[(int)Session["j"]].getOrderNo() %>">删除</a></td>
				</tr>
                <%} %>
				</table>
			</div>
